...
// To jest program z podziałem na 8 perceptronów i dla każdej liczby wątków ma ten sam wynik więc jest ok.


#define FEATURES 8  // Liczba cech (atrybutów) w danych

#define NUM_PERCEPTRONS 8
...
int main(int argc, char *argv[]) {
    ...
    // Inicjalizacja 10 perceptronów z losowymi wagami
    Perceptron perceptrons[NUM_PERCEPTRONS];
    #pragma omp parallel for
    for (int p = 0; p < NUM_PERCEPTRONS; p++) {
        for (int i = 0; i < FEATURES; i++) {
            perceptrons[p].weights[i] = (rand() % 1000) / 1000.0;  // Losowe liczby z przedziału [0, 1)
        }
        perceptrons[p].bias = (rand() % 1000) / 1000.0;
    }

    // Uczenie każdego perceptronu na odpowiednim fragmencie zbioru treningowego przez określoną liczbę epok
    ...
    for (int epoch = 0; epoch < numEpochs; epoch++) {
        for (int p = 0; p < NUM_PERCEPTRONS; p++) {
            for (int i = 0; i < trainingSize / NUM_PERCEPTRONS; i++) {
                trainPerceptron(&perceptrons[p], trainingSet[i + p * (trainingSize / NUM_PERCEPTRONS)], learningRate);
            }
        }
    }
    ...
    // Testowanie każdego perceptronu na zbiorze testowym
    ...
    for (int i = 0; i < testingSize; i++) {
        ...
        for (int p = 0; p < NUM_PERCEPTRONS; p++) {
            majorityVote += predict(perceptrons[p], testingSet[i].features);
        }

        majorityVote = (majorityVote >= (NUM_PERCEPTRONS / 2)) ? 1 : 0;

        printf("%d\t\t%d\t\t", realLabel, majorityVote);

        if (realLabel == majorityVote) {
            printf("Correct\n");
            correctPredictions++;
        } else {
            printf("Incorrect\n");
